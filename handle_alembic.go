...
//handleAddAlembic 함수는 URL에 objectID를 붙여서 /addalembic-item 페이지로 redirect 한다.
func handleAddAlembic(w http.ResponseWriter, r *http.Request) {
	...
	http.Redirect(w, r, fmt.Sprintf("addalembic-item?objectid=%s", objectID), http.StatusSeeOther)

}

//handleAddAlembicItem 함수는 DB에 저장할 Alembic 파일의 정보를 입력하는 페이지이다.
func handleAddAlembicItem(w http.ResponseWriter, r *http.Request) {
	...
	err = TEMPLATES.ExecuteTemplate(w, "addalembic-item", rcp)
	if err != nil {
		http.Error(w, err.Error(), http.StatusInternalServerError)
		return
	}
...
// handleUploadAlembicItem 함수는 /addalembic-item에서 입력한 정보를 DB에 저장하는 함수이다.
func handleUploadAlembicItem(w http.ResponseWriter, r *http.Request) {
	_, err := GetTokenFromHeader(w, r)
	if err != nil {
		http.Redirect(w, r, "/signin", http.StatusSeeOther)
		return
	...
	if err != nil {
		if IsDup(err) { // 동일한 ID의 도큐먼트를 업로드하려고 하면, 새로운 ID의 페이지로 리다이렉트한다.
			http.Redirect(w, r, "/addalembic", http.StatusSeeOther)
			return
		}
		http.Error(w, err.Error(), http.StatusInternalServerError)
		return
	}
	http.Redirect(w, r, fmt.Sprintf("/addalembic-file?objectid=%s", objectID), http.StatusSeeOther)
}

// handleAddAlembicFile 함수는 Alembic 파일을 추가하는 페이지 이다.
func handleAddAlembicFile(w http.ResponseWriter, r *http.Request) {
	token, err := GetTokenFromHeader(w, r)
	if err != nil {
		http.Redirect(w, r, "/signin", http.StatusSeeOther)
		return
	...
	w.Header().Set("Content-Type", "text/html")
	err = TEMPLATES.ExecuteTemplate(w, "addalembic-file", rcp)
	if err != nil {
		http.Error(w, err.Error(), http.StatusInternalServerError)
		return
	}
...
// handleUploadAlembicFile 함수는 Alembic 파일을 DB에 업로드하는 페이지를 연다. dropzone에 파일을 올릴 경우 실행된다.
func handleUploadAlembicFile(w http.ResponseWriter, r *http.Request) {
	_, err := GetTokenFromHeader(w, r)
	if err != nil {
		http.Redirect(w, r, "/signin", http.StatusSeeOther)
		return
