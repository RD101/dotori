...
// handleAddUSD 함수는 URL에 objectID를 붙여서 /addusd-item 페이지로 redirect한다.
func handleAddUSD(w http.ResponseWriter, r *http.Request) {
	...
	http.Redirect(w, r, fmt.Sprintf("/addusd-item?objectid=%s", objectID), http.StatusSeeOther)
}

// handleAddUSDItem 함수는 DB에 저장할 usd 파일의 정보를 입력하는 페이지 이다.
func handleAddUSDItem(w http.ResponseWriter, r *http.Request) {
...
// handleUploadUSDItem 함수는 /addusd-item에서 입력한 정보를 DB에 저장하는 함수이다.
func handleUploadUSDItem(w http.ResponseWriter, r *http.Request) {
	_, err := GetTokenFromHeader(w, r)
	if err != nil {
		http.Redirect(w, r, "/signin", http.StatusSeeOther)
		return
	...
	if err != nil {
		if IsDup(err) { // 동일한 ID의 도큐먼트를 업로드하려고 하면, 새로운 ID의 페이지로 리다이렉트한다.
			http.Redirect(w, r, "/addusd", http.StatusSeeOther)
			return
		}
		http.Error(w, err.Error(), http.StatusInternalServerError)
		return
	}
	http.Redirect(w, r, fmt.Sprintf("/addusd-file?objectid=%s", objectID), http.StatusSeeOther)
}

// handleAddUSDFile 함수는 usd 파일을 추가하는 페이지 이다.
func handleAddUSDFile(w http.ResponseWriter, r *http.Request) {
	token, err := GetTokenFromHeader(w, r)
	if err != nil {
		http.Redirect(w, r, "/signin", http.StatusSeeOther)
		return
...
// handleUploadUSD 함수는 usd파일을 DB에 업로드하는 페이지를 연다. dropzone에 파일을 올릴 경우 실행된다.
func handleUploadUSDFile(w http.ResponseWriter, r *http.Request) {
	_, err := GetTokenFromHeader(w, r)
	if err != nil {
		http.Redirect(w, r, "/signin", http.StatusSeeOther)
		return
	...
	uploadUSDFile(w, r, objectID)
}

// uploadUSDFile 함수는 USD 파일 정보를 DB에 업로드하고 파일을 storage에 복사한다.
func uploadUSDFile(w http.ResponseWriter, r *http.Request, objectID string) {
	//mongoDB client 연결
	client, err := mongo.NewClient(options.Client().ApplyURI(*flagMongoDBURI))
	if err != nil {
		http.Error(w, err.Error(), http.StatusInternalServerError)
	...
	for _, files := range r.MultipartForm.File {
		for _, f := range files {
			...
			case "application/octet-stream":
				...
				if ext != ".usd" && ext != ".usdc" && ext != ".usda" && ext != ".usdz" { // USD파일 외에는 허용하지 않는다.
...
func handleUploadUSDCheckData(w http.ResponseWriter, r *http.Request) {
	...
	rcp.Item = item
	if !item.ThumbImgUploaded || !item.ThumbClipUploaded || !item.DataUploaded {
		err = TEMPLATES.ExecuteTemplate(w, "checkusd-file", rcp)
		if err != nil {
			http.Error(w, err.Error(), http.StatusInternalServerError)
			return
		}
	...
	http.Redirect(w, r, "/addusd-success", http.StatusSeeOther)
}

func handleAddUSDSuccess(w http.ResponseWriter, r *http.Request) {
	token, err := GetTokenFromHeader(w, r)
	if err != nil {
		http.Redirect(w, r, "/signin", http.StatusSeeOther)
		return
...
func handleEditUSD(w http.ResponseWriter, r *http.Request) {
	token, err := GetTokenFromHeader(w, r)
	if err != nil {
		http.Redirect(w, r, "/signin", http.StatusSeeOther)
		return
...
//handleEditUSDSubmit 함수는 usd아이템을 수정하는 페이지에서 UPDATE버튼을 누르면 작동하는 함수다.
func handleEditUSDSubmit(w http.ResponseWriter, r *http.Request) {
	_, err := GetTokenFromHeader(w, r)
	if err != nil {
		http.Redirect(w, r, "/signin", http.StatusSeeOther)
		return
	...
	http.Redirect(w, r, "/editusd-success", http.StatusSeeOther)
}

func handleEditUSDSuccess(w http.ResponseWriter, r *http.Request) {
	token, err := GetTokenFromHeader(w, r)
	if err != nil {
		http.Redirect(w, r, "/signin", http.StatusSeeOther)
		return
