...
// handleAddHoudini 함수는 URL에 objectID를 붙여서 /addhoudini-item 페이지로 redirect한다.
func handleAddHoudini(w http.ResponseWriter, r *http.Request) {
	...
	http.Redirect(w, r, fmt.Sprintf("/addhoudini-item?objectid=%s", objectID), http.StatusSeeOther)
}

// handleAddHoudiniItem 함수는 DB에 저장할 Houdini 파일의 정보를 입력하는 페이지 이다.
func handleAddHoudiniItem(w http.ResponseWriter, r *http.Request) {
	...
	err = TEMPLATES.ExecuteTemplate(w, "addhoudini-item", rcp)
	if err != nil {
		http.Error(w, err.Error(), http.StatusInternalServerError)
		return
	}
...
// handleUploadHoudiniItem 함수는 /addhoudini-item에서 입력한 정보를 DB에 저장하는 함수이다.
func handleUploadHoudiniItem(w http.ResponseWriter, r *http.Request) {
	_, err := GetTokenFromHeader(w, r)
	if err != nil {
		http.Redirect(w, r, "/signin", http.StatusSeeOther)
		return
	...
	if err != nil {
		if IsDup(err) { // 동일한 ID의 도큐먼트를 업로드하려고 하면, 새로운 ID의 페이지로 리다이렉트한다.
			http.Redirect(w, r, "/addhoudini", http.StatusSeeOther)
			return
		}
		http.Error(w, err.Error(), http.StatusInternalServerError)
		return
	}
	http.Redirect(w, r, fmt.Sprintf("/addhoudini-file?objectid=%s", objectID), http.StatusSeeOther)
}

// handleAddHoudiniFile 함수는 Houdini 파일을 추가하는 페이지 이다.
func handleAddHoudiniFile(w http.ResponseWriter, r *http.Request) {
	token, err := GetTokenFromHeader(w, r)
	if err != nil {
		http.Redirect(w, r, "/signin", http.StatusSeeOther)
		return
	...
	w.Header().Set("Content-Type", "text/html")
	err = TEMPLATES.ExecuteTemplate(w, "addhoudini-file", rcp)
	if err != nil {
		http.Error(w, err.Error(), http.StatusInternalServerError)
		return
	}
...
// handleUploadHoudini 함수는 Houdini파일을 DB에 업로드하는 페이지를 연다. dropzone에 파일을 올릴 경우 실행된다.
func handleUploadHoudiniFile(w http.ResponseWriter, r *http.Request) {
	_, err := GetTokenFromHeader(w, r)
	if err != nil {
		http.Redirect(w, r, "/signin", http.StatusSeeOther)
		return
	...
	uploadHoudiniFile(w, r, objectID)
}

// uploadHoudiniFile 함수는 Houdini 파일 정보를 DB에 업로드하고 파일을 storage에 복사한다.
func uploadHoudiniFile(w http.ResponseWriter, r *http.Request, objectID string) {
	//mongoDB client 연결
	client, err := mongo.NewClient(options.Client().ApplyURI(*flagMongoDBURI))
	if err != nil {
		http.Error(w, err.Error(), http.StatusInternalServerError)
...
func handleUploadHoudiniCheckData(w http.ResponseWriter, r *http.Request) {
	...
	rcp.Item = item
	if !item.ThumbImgUploaded || !item.ThumbClipUploaded || !item.DataUploaded {
		err = TEMPLATES.ExecuteTemplate(w, "checkhoudini-file", rcp)
		if err != nil {
			http.Error(w, err.Error(), http.StatusInternalServerError)
			return
		}
	...
	http.Redirect(w, r, "/addhoudini-success", http.StatusSeeOther)
}

func handleAddHoudiniSuccess(w http.ResponseWriter, r *http.Request) {
	token, err := GetTokenFromHeader(w, r)
	if err != nil {
		http.Redirect(w, r, "/signin", http.StatusSeeOther)
		return
	...
	w.Header().Set("Content-Type", "text/html")
	err = TEMPLATES.ExecuteTemplate(w, "addhoudini-success", rcp)
	if err != nil {
		http.Error(w, err.Error(), http.StatusInternalServerError)
		return
	}
...
func handleEditHoudini(w http.ResponseWriter, r *http.Request) {
	token, err := GetTokenFromHeader(w, r)
	if err != nil {
		http.Redirect(w, r, "/signin", http.StatusSeeOther)
		return
	...
	err = TEMPLATES.ExecuteTemplate(w, "edithoudini", rcp)
	if err != nil {
		http.Error(w, err.Error(), http.StatusInternalServerError)
		return
	}
...
//handleEditHoudiniSubmit 함수는 houdini아이템을 수정하는 페이지에서 UPDATE버튼을 누르면 작동하는 함수다.
func handleEditHoudiniSubmit(w http.ResponseWriter, r *http.Request) {
	_, err := GetTokenFromHeader(w, r)
	if err != nil {
		http.Redirect(w, r, "/signin", http.StatusSeeOther)
		return
	...
	http.Redirect(w, r, "/edithoudini-success", http.StatusSeeOther)
}

func handleEditHoudiniSuccess(w http.ResponseWriter, r *http.Request) {
	token, err := GetTokenFromHeader(w, r)
	if err != nil {
		http.Redirect(w, r, "/signin", http.StatusSeeOther)
		return
	...
	w.Header().Set("Content-Type", "text/html")
	err = TEMPLATES.ExecuteTemplate(w, "edithoudini-success", rcp)
	if err != nil {
		http.Error(w, err.Error(), http.StatusInternalServerError)
		return
	}
