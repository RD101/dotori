...
//handleAddKatana 함수는 URL에 objectID를 붙여서 /addkatana-item 페이지로 redirect 한다.
func handleAddKatana(w http.ResponseWriter, r *http.Request) {
	...
	http.Redirect(w, r, fmt.Sprintf("addkatana-item?objectid=%s", objectID), http.StatusSeeOther)

}

//handleAddKatanaItem 함수는 DB에 저장할 Katana 파일의 정보를 입력하는 페이지이다.
func handleAddKatanaItem(w http.ResponseWriter, r *http.Request) {
	...
	err = TEMPLATES.ExecuteTemplate(w, "addkatana-item", rcp)
	if err != nil {
		http.Error(w, err.Error(), http.StatusInternalServerError)
		return
	}
...
// handleUploadKatanaItem 함수는 /addkatana-item에서 입력한 정보를 DB에 저장하는 함수이다.
func handleUploadKatanaItem(w http.ResponseWriter, r *http.Request) {
	_, err := GetTokenFromHeader(w, r)
	if err != nil {
		http.Redirect(w, r, "/signin", http.StatusSeeOther)
		return
	...
	if err != nil {
		if IsDup(err) { // 동일한 ID의 도큐먼트를 업로드하려고 하면, 새로운 ID의 페이지로 리다이렉트한다.
			http.Redirect(w, r, "/addkatana", http.StatusSeeOther)
			return
		}
		http.Error(w, err.Error(), http.StatusInternalServerError)
		return
	}
	http.Redirect(w, r, fmt.Sprintf("/addkatana-file?objectid=%s", objectID), http.StatusSeeOther)
}

// handleAddKatanaFile 함수는 Katana 파일을 추가하는 페이지 이다.
func handleAddKatanaFile(w http.ResponseWriter, r *http.Request) {
	token, err := GetTokenFromHeader(w, r)
	if err != nil {
		http.Redirect(w, r, "/signin", http.StatusSeeOther)
		return
	...
	w.Header().Set("Content-Type", "text/html")
	err = TEMPLATES.ExecuteTemplate(w, "addkatana-file", rcp)
	if err != nil {
		http.Error(w, err.Error(), http.StatusInternalServerError)
		return
	}
...
// handleUploadKatanaFile 함수는 Katana 파일을 DB에 업로드하는 페이지를 연다. dropzone에 파일을 올릴 경우 실행된다.
func handleUploadKatanaFile(w http.ResponseWriter, r *http.Request) {
	_, err := GetTokenFromHeader(w, r)
	if err != nil {
		http.Redirect(w, r, "/signin", http.StatusSeeOther)
		return
	...
	uploadKatanaFile(w, r, objectID)
}

// uploadKatanaFile 함수는 Katana 파일 정보를 DB에 업로드하고 파일을 storage에 복사한다.
func uploadKatanaFile(w http.ResponseWriter, r *http.Request, objectID string) {
	//mongoDB client 연결
	client, err := mongo.NewClient(options.Client().ApplyURI(*flagMongoDBURI))
	if err != nil {
		http.Error(w, err.Error(), http.StatusInternalServerError)
	...
	for _, files := range r.MultipartForm.File {
		for _, f := range files {
			...
			case "application/octet-stream":
				...
				if ext != ".katana" { // .abc 외에는 허용하지 않는다.
...
// handleUploadKatanaCheckData 함수는 필요한 파일(썸네일, data 파일 등)을 추가했는지 체크한다.
func handleUploadKatanaCheckData(w http.ResponseWriter, r *http.Request) {
	token, err := GetTokenFromHeader(w, r)
	if err != nil {
		http.Redirect(w, r, "/signin", http.StatusSeeOther)
		return
	...
	rcp.Item = item
	if !item.ThumbImgUploaded || !item.ThumbClipUploaded || !item.DataUploaded {
		err = TEMPLATES.ExecuteTemplate(w, "checkkatana-file", rcp)
		if err != nil {
			http.Error(w, err.Error(), http.StatusInternalServerError)
			return
		}
	...
	http.Redirect(w, r, "/addkatana-success", http.StatusSeeOther)
}

// handleAddKatanaSuccess 함수는 add katana item을 성공했다는 페이지를 연다.
func handleAddKatanaSuccess(w http.ResponseWriter, r *http.Request) {
	token, err := GetTokenFromHeader(w, r)
	if err != nil {
		http.Redirect(w, r, "/signin", http.StatusSeeOther)
		return
	...
	w.Header().Set("Content-Type", "text/html")
	err = TEMPLATES.ExecuteTemplate(w, "addkatana-success", rcp)
	if err != nil {
		http.Error(w, err.Error(), http.StatusInternalServerError)
		return
	}
...
// handleEditKatana 함수는 katana item 정보를 수정할 수 있는 페이지를 연다. edit 버튼을 누를 때 실행된다.
func handleEditKatana(w http.ResponseWriter, r *http.Request) {
	token, err := GetTokenFromHeader(w, r)
	if err != nil {
		http.Redirect(w, r, "/signin", http.StatusSeeOther)
		return
	...
	err = TEMPLATES.ExecuteTemplate(w, "editkatana", rcp)
	if err != nil {
		http.Error(w, err.Error(), http.StatusInternalServerError)
		return
	}
...
//handleEditKatanaSubmit 함수는 katana 아이템을 수정하는 페이지에서 UPDATE 버튼을 누르면 작동하는 함수다.
func handleEditKatanaSubmit(w http.ResponseWriter, r *http.Request) {
	_, err := GetTokenFromHeader(w, r)
	if err != nil {
		http.Redirect(w, r, "/signin", http.StatusSeeOther)
		return
	...
	http.Redirect(w, r, "/editkatana-success", http.StatusSeeOther)
}

// handleEditKatanaSuccess 함수는 katana item 정보 수정을 성공했다는 페이지를 연다.
func handleEditKatanaSuccess(w http.ResponseWriter, r *http.Request) {
	token, err := GetTokenFromHeader(w, r)
	if err != nil {
		http.Redirect(w, r, "/signin", http.StatusSeeOther)
		return
	...
	w.Header().Set("Content-Type", "text/html")
	err = TEMPLATES.ExecuteTemplate(w, "editkatana-success", rcp)
	if err != nil {
		http.Error(w, err.Error(), http.StatusInternalServerError)
		return
	}
