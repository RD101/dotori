...
func handleUploadLutFile(w http.ResponseWriter, r *http.Request) {
	...
	uploadLutFile(w, r, objectID)
}

// uploadLutFile 함수는 Lut 파일 정보를 DB에 업로드하고 파일을 storage에 복사한다.
func uploadLutFile(w http.ResponseWriter, r *http.Request, objectID string) {
	//mongoDB client 연결
	client, err := mongo.NewClient(options.Client().ApplyURI(*flagMongoDBURI))
	if err != nil {
		http.Error(w, err.Error(), http.StatusInternalServerError)
