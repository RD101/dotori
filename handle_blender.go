...
func handleUploadBlenderFile(w http.ResponseWriter, r *http.Request) {
	...
	uploadBlenderFile(w, r, objectID)
}

// uploadBlenderFile 함수는 Blender 파일 정보를 DB에 업로드하고 파일을 storage에 복사한다.
func uploadBlenderFile(w http.ResponseWriter, r *http.Request, objectID string) {
	//mongoDB client 연결
	client, err := mongo.NewClient(options.Client().ApplyURI(*flagMongoDBURI))
	if err != nil {
		http.Error(w, err.Error(), http.StatusInternalServerError)
...
func handleUploadBlenderCheckData(w http.ResponseWriter, r *http.Request) {
	...
	rcp.Item = item
	if !item.ThumbImgUploaded || !item.ThumbClipUploaded || !item.DataUploaded {
		err = TEMPLATES.ExecuteTemplate(w, "checkblender-file", rcp)
		if err != nil {
			http.Error(w, err.Error(), http.StatusInternalServerError)
			return
		}
